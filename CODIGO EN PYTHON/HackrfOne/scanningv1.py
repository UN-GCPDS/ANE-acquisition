...
class Scanner:
    def __init__(self, start=88e6, stop=108e6, sample_rate=10e6, duration=1, num_rep=1):
        self.start = start
        self.stop = stop
    ...
    def scan(self):
        ...
        for i in range(self.num_rep):
            while freq <= self.stop:
                ...
                iq_samples = high100(iq_samples, fs=256)

                avg_pwr = np.mean(np.abs(iq_samples)**2)

                psd, freqs = plt.psd(iq_samples, NFFT=2048, Fs=self.sample_rate/1e6, Fc=hackrf.center_freq/1e6)
                plt.close()

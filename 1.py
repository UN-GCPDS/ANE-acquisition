...
class ScannerApp(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        default_values = {'ppm': '5', 'gain': '20', 'threshold': '0.75', 'lnb lo': '-125000000', 'start': '87000000', 'stop': '108000000', 'step': '100000'}
            ...
            try:
                ...
                if peak_indices:
                    ...
                    if peak_psd >= radio_psd_threshold:  # Check if the PSD value is above the radio station threshold
                        ...
                        # Usar un ancho de banda de 200 kHz para el waterfall
                        waterfall_bandwidth = 200e3
                        NFFT = int(sample_rate / 48 / waterfall_bandwidth * 1024)
                        frequency_offset = freq - sample_rate / 48 / 2
                        # Generar el plot del waterfall
                        plt.figure()
                        specgram(iq_samples, NFFT=NFFT, Fs=sample_rate / 48, noverlap=NFFT//2, cmap='viridis', mode='magnitude', scale='dB', vmin=-60, vmax=0)
                        plt.title(f'Waterfall Plot - {freq / 1e6} MHz')
                        plt.xlabel('Time (s)')
                        plt.ylabel('Frequency (Hz)')
                        plt.ylim(peak_frequency - waterfall_bandwidth / 2, peak_frequency + waterfall_bandwidth / 2)
                        plt.show()
